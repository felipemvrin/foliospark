      ...
      {hasRemoteProjects && projects.length === 0 ? (
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-2">
          {projects.map((project, index) => {
            const projectHref = getSafeExternalHref(project.url)
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  {projectHref ? (
                    <a href={projectHref} target="_blank" rel="noopener noreferrer" onClick={() => void trackAnalyticsEvent({ name: 'outbound_click', properties: { destination: 'github_repository' } })} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[var(--border)] text-[var(--muted)] transition hover:text-[var(--foreground)]" aria-label={`Open ${project.repository} on GitHub`}>
